...
@Directive({
    selector: 'p-tree[kPrimeTreePropagation]',
})
export class PrimeTreePropagation implements OnInit, DoCheck, OnChanges {

    _selectionDiffer : IterableDiffer<PrimeTreeNode>;
    ...
    public updateNodeState(node : PrimeTreeNode, addToSelection : boolean = false)
    {
        if (node)
        {
    ...
    private _hasParentThatIsSelected(node : PrimeTreeNode) : boolean {
    ...
    private _applyChanges(changes: IterableChanges<PrimeTreeNode>) {
    ...
    private _propagateUpForDefaultMode(node: PrimeTreeNode, select: boolean, handledList : PrimeTreeNode[]) : { selectionChanged : boolean} {
    ...
    private _propagateDownDefaultMode(node: PrimeTreeNode, select: boolean, handledList : PrimeTreeNode[]) : { selectionChanged : boolean} {
        const result = {selectionChanged: false};
    ...
    private _propagateDownForPreventNestedMode(node: PrimeTreeNode, select: boolean, handledList : PrimeTreeNode[])  : { selectionChanged : boolean} {
        const result = {selectionChanged: false};
    ...
    private _propagateUpForVisualMode(node: PrimeTreeNode, select: boolean, handledList : PrimeTreeNode[]) {
        if (handledList.indexOf(node) !== -1)
        {
            return;
        }
